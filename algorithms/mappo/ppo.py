...
from algorithms.utils.feature_process import Feature_Process_Actor, Feature_Process_Critic
...
class R_Actor(nn.Module):
    ...
    def __init__(self, args, obs_space, action_space, device=torch.device("cuda")):
        ...
        if args.use_feature:
            self.base = Feature_Process_Actor(args, obs_shape, args.hidden_size, self.device)
        else:
...
class R_Critic(nn.Module):
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cuda")):
        ...
        if args.use_feature:
            self.base = Feature_Process_Critic(args, cent_obs_shape, args.hidden_size, device)
        else:
